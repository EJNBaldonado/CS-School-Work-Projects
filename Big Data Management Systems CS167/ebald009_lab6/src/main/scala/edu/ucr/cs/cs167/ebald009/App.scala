    ...
    try {
      ...
      command match {
        case "count-all" =>
          ...
          val count: Long = val count: Long : parsedLines.count() // TODO 2: count total number of records in the file on `parsedLines`
        ...
        case "time-filter" =>
          ...
          val filteredLines: RDD[Array[String]] = partsedLines.filter(row => row(2) == {
        ...
        case "sum-bytes-by-code" =>
          ...
          val loglinesByCode: RDD[(String, Long)] = parsedLines.map(row=> (row(5),row(6).toLong))
        ...
        case "avg-bytes-by-code" =>
          ...
          val counts: Map[String, Long] = loglinesByCode.aggregateByKey()
